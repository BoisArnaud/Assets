...
public class PlayerInventory : MonoBehaviour
{
    ...
    void EquipWeapon(Item item)
    {
        ...
        {
			Transform t = Instantiate (item.getItemModel().transform, weaponSlot.position, weaponSlot.rotation) as Transform;
			GameObject Clone = t.gameObject;
			Clone.name = item.getName()+"_"+item.getIdItem();
    ...
    void UnEquipWeapon(Item item)
    {
        if (item.itemType == ItemType.Weapon)
        {
			Destroy(weaponSlot.FindChild(item.getName()+"_"+item.getIdItem()).gameObject);
        }
    }
    ...
    void UpdateHPBar()
    {
        hpText.text = (ps.getHealth() + "/" + ps.getHealthMax());
		if (ps.getHealthMax() != 0) {
			float fillAmount = ps.getHealth () / ps.getHealthMax ();
    ...
    void UpdateManaBar()
    {
		manaText.text = (ps.getMana() + "/" + ps.getManaMax());	
		if (ps.getManaMax () != 0) {
			float fillAmount = ps.getMana () / ps.getManaMax ();
    ...
    public void OnGearItem(Item item)
    {
        ...
        {
            if (item.itemAttributes[i].attributeName == "Health")
				ps.setHealthMax(ps.getHealthMax()+item.itemAttributes[i].attributeValue);
            if (item.itemAttributes[i].attributeName == "Mana")
			...
			if(item.itemAttributes[i].attributeName == "END"){
				ps.setEndurance(ps.getEndurance()+item.itemAttributes[i].attributeValue);
			}
			if(item.itemAttributes[i].attributeName == "FOR"){
				ps.setForce(ps.getForce()+item.itemAttributes[i].attributeValue);
			}
			if(item.itemAttributes[i].attributeName == "INT"){
				ps.setIntelligence(ps.getIntelligence()+item.itemAttributes[i].attributeValue);
			}
			if(item.itemAttributes[i].attributeName == "AGI"){
				ps.setAgilite(ps.getAgilite()+item.itemAttributes[i].attributeValue);
			}
